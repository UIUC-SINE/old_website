...
def poop():
    ...
    meas_focus = get_measurements(sources=sources, psfs=psfs_focus, real=True)
    meas_mid = get_measurements(sources=sources, psfs=psfs_mid, real=True)
    meas_outer = get_measurements(sources=sources, psfs=psfs_outer, real=True)
    meas_csbs = get_measurements(sources=sources, psfs=psfs_csbs, real=True)

    # meas_focus = add_noise(meas_focus, model='gaussian', dbsnr=30)
    # meas_mid = add_noise(meas_mid, model='gaussian', dbsnr=30)
    # meas_outer = add_noise(meas_outer, model='gaussian', dbsnr=30)
    # meas_csbs = add_noise(meas_csbs, model='gaussian', dbsnr=30)
    ...
    recon_focus = tikhonov(psfs=psfs_focus, measurements=meas_focus, tikhonov_lam=1e-4)
    recon_mid = tikhonov(psfs=psfs_mid, measurements=meas_mid, tikhonov_lam=1e-4)
    recon_outer = tikhonov(psfs=psfs_outer, measurements=meas_outer, tikhonov_lam=1e-4)
    recon_csbs = tikhonov(psfs=psfs_csbs, measurements=meas_csbs, tikhonov_lam=1e-4)
    ...
    sources -= np.mean(sources, axis=(1, 2))[:, np.newaxis, np.newaxis]

    result_focus = np.sum(compare_ssim(sources, recon_focus))
    result_mid = np.sum(compare_ssim(sources, recon_mid))
    result_outer = np.sum(compare_ssim(sources, recon_outer))
